...
public class ApplyChangesHandler implements ActionHandler
{
	...
	public void execute (ExecutionContext executionContext) throws Exception
	{
		...
		Long systemId = (Long) executionContext.getVariable(Constants.RECERTIFICATION_SYSTEM_ID_VAR);

		RecertificationProcess process = ejb.getRecertificationProcess(processId);
		RecertifiedInformationSystem ru = ejb.getRecertifiedInformationSystem(systemId);
		ru.setStatus(ProcessStatus.FINISHED);
		ejb.update(ru);
		ejb.applyInformationSystemRecertificationProcess(ru);
